# UrbanFlux — Infrastructure Cascades & Resilience Literature
...
The goal is to provide rigorous scientific justification for why UrbanFlux models cascading stress propagation from road closures to healthcare access.
...
- **Application to UrbanFlux**: Justifies the multi-layer dependency model linking physical road status to secondary corridor saturation and hospital response times.
...
- **Application to UrbanFlux**: Directly adopted into our `Criticality Model` (`docs/simulation/criticality_model.md`) combining betweenness, flow volume, and service dependency.
...
- **Application to UrbanFlux**: Directly validated in our Part B simulation demo (`examples/network_disruption_demo.py`), where a single bridge closure increases hospital approach travel time by $+244.1\\%$ ($+17.0\\text{ minutes}$).
...
- **Application to UrbanFlux**: Provides the mathematical metric for future intervention and mitigation comparison (evaluating which rerouting policy minimizes the loss-of-performance area).

---

## 3. Scientific Evidence Ledger

| Scientific Claim | Author & Year | Publication Venue | How UrbanFlux Uses It | Confidence Level |
|---|---|---|---|---|
